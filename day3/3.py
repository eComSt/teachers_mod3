...
class MainWindow(CTk):
    ...
    def __init__(self,title):
        ...
        self.checkbox_frame = MyCheckboxFrame(self, title='Выбери цвет', values=('Красный', 'Синий', 'Зеленый','Черный','Белый','Серобуромалиновый'))
        self.checkbox_frame.grid(row=0, column=0,  padx=10, pady=10, sticky='nsew')
        # создаем и размещаем фрейм с радиокнопками
        self.radiobutton_frame = MyRadiobuttonFrame(self, title='Выбери фигуру', values=('Круг', 'Квадрат','Треугольник','Ромбододекаэдр'))
...
class MyRadiobuttonFrame(CTkFrame):
    ...
    def __init__(self, master, title, values):
        ...
        # задаем сеточную конфигурацию
        self.grid_columnconfigure(0, weight=1)
        # список для хранения радиокнопок
        self.radiobuttons = []
        ...
        for i in range(len(values)):
            ...
            # добавляем созданную радиокнопку в список
